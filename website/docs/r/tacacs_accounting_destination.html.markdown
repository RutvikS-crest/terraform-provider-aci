...
resource "aci_tacacs_accounting_destination" "example" {
  tacacs_monitoring_destination_group_dn  = aci_tacacs_monitoring_destination_group.example.id
  host  = "example"
  port  = "example"
  ...
  file_rs_a_remote_host_to_epg = aci_resource.example.id

  file_rs_a_remote_host_to_epp = aci_resource.example.id
}
```
