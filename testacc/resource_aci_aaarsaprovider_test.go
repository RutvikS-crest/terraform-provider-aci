...
func TestAccAciRSAProvider_Update(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: CreateAccRSAProviderUpdatedAttr(rName, "retries", "5"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAciRSAProviderExists(resourceName, &rsa_provider_updated),
...
func TestAccAciRSAProvider_Negative(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config:      CreateAccRSAProviderUpdatedAttr(rName, "key", randomValue),
				ExpectError: regexp.MustCompile(`expected(.)+ to be one of (.)+, got(.)+`),
			},
			...
			{
				Config:      CreateAccRSAProviderUpdatedAttr(rName, "monitoring_password", randomValue),
				ExpectError: regexp.MustCompile(`expected(.)+ to be one of (.)+, got(.)+`),
			},

			{
				Config:      CreateAccRSAProviderUpdatedAttr(rName, "monitoring_user", randomValue),
				ExpectError: regexp.MustCompile(`expected(.)+ to be one of (.)+, got(.)+`),
			},
			...
			{
				Config:      CreateAccRSAProviderUpdatedAttr(rName, "retries", "-1"),
				ExpectError: regexp.MustCompile(`out of range`),
			},
			{
			...
			{
				Config:      CreateAccRSAProviderUpdatedAttr(rName, "timeout", "-1"),
				ExpectError: regexp.MustCompile(`out of range`),
			},
			{
...
func CreateAccRSAProviderConfigWithRequiredParams(rName string) string {
	fmt.Println("=== STEP  testing rsa_provider creation with updated naming arguments")
