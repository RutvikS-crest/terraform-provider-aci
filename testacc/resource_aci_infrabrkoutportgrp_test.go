...
func TestAccAciLeafBreakoutPortGroup_Basic(t *testing.T) {
	...
	resourceName := "aci_leaf_breakout_port_group.testacc"
...
func TestAccAciLeafBreakoutPortGroup_Update(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: CreateAccLeafBreakoutPortGroupUpdatedAttr(rName, "brkoutMap", "100g-4x"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAciLeafBreakoutPortGroupExists(resourceName, &leaf_breakout_port_group_updated),
					resource.TestCheckResourceAttr(resourceName, "brkoutMap", "100g-4x"),
					testAccCheckAciLeafBreakoutPortGroupIdEqual(&leaf_breakout_port_group_default, &leaf_breakout_port_group_updated),
				),
			},
			{
				Config: CreateAccLeafBreakoutPortGroupUpdatedAttr(rName, "brkoutMap", "10g-4x"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAciLeafBreakoutPortGroupExists(resourceName, &leaf_breakout_port_group_updated),
					resource.TestCheckResourceAttr(resourceName, "brkoutMap", "10g-4x"),
					testAccCheckAciLeafBreakoutPortGroupIdEqual(&leaf_breakout_port_group_default, &leaf_breakout_port_group_updated),
				),
			},
			{
				Config: CreateAccLeafBreakoutPortGroupUpdatedAttr(rName, "brkoutMap", "25g-4x"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAciLeafBreakoutPortGroupExists(resourceName, &leaf_breakout_port_group_updated),
					resource.TestCheckResourceAttr(resourceName, "brkoutMap", "25g-4x"),
					testAccCheckAciLeafBreakoutPortGroupIdEqual(&leaf_breakout_port_group_default, &leaf_breakout_port_group_updated),
				),
			},
			{
				Config: CreateAccLeafBreakoutPortGroupUpdatedAttr(rName, "brkoutMap", "50g-8x"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAciLeafBreakoutPortGroupExists(resourceName, &leaf_breakout_port_group_updated),
					resource.TestCheckResourceAttr(resourceName, "brkoutMap", "50g-8x"),
					testAccCheckAciLeafBreakoutPortGroupIdEqual(&leaf_breakout_port_group_default, &leaf_breakout_port_group_updated),
				),
			},
			{
...
func CreateAccLeafBreakoutPortGroupConfigWithRequiredParams(rName string) string {
	fmt.Println("=== STEP  testing leaf_breakout_port_group creation with required arguements only")
...
func CreateAccLeafBreakoutPortGroupConfigUpdatedName(rName string) string {
	fmt.Println("=== STEP  testing leaf_breakout_port_group creation with with Invalid Name: %s", rName)
	resource := fmt.Sprintf(`
