		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "provider_type", ""),
...
func TestAccAciDuoProviderGroup_Negative(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config:      CreateAccDuoProviderGroupUpdatedAttr(rName, "ldap_group_map_ref", randomValue),
				ExpectError: regexp.MustCompile(`expected(.)+ to be one of (.)+, got(.)+`),
			},
			...
			// TODO: add unspecified case for "sec_fac_auth_methods" if applicable
...
func CreateAccDuoProviderGroupConfigWithRequiredParams(rName string) string {
	fmt.Println("=== STEP  testing duo_provider_group creation with updated naming arguments")
